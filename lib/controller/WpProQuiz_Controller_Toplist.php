...
class WpProQuiz_Controller_Toplist extends WpProQuiz_Controller_Controller
{
    ...
    private function handleAddInToplist(WpProQuiz_Model_Quiz $quiz)
    {
        if (!wp_verify_nonce($this->_post['token'], 'wpProQuiz_toplist')) {
            return array('text' => __('Ocorreu um erro.', 'wp-pro-quiz'), 'clear' => true);
        }
        ...
        if ($userId > 0) {
            if ($toplistMapper->countUser($quizId, $userId, $clearTime)) {
                return array('text' => __('Você não pode entrar novamente.', 'wp-pro-quiz'), 'clear' => true);
            }
            ...
            if (empty($name) || empty($email) || filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
                return array('text' => __('Nenhum nome ou e-mail informado.', 'wp-pro-quiz'), 'clear' => false);
            }

            if (strlen($name) > 15) {
                return array('text' => __('Seu nome não pode exceder 15 caracteres.', 'wp-pro-quiz'), 'clear' => false);
            }
            ...
            if ($quiz->isToplistDataCaptcha()) {
                ...
                if ($captcha->isSupported()) {
                    if (!$captcha->check($prefix, $captchaAnswer)) {
                        return array('text' => __('Você digitou o código captcha errado.', 'wp-pro-quiz'), 'clear' => false);
                    }
                }
            }
        }
    ...
    public static function ajaxAddInToplist($data)
    {
        ...
        if ($r === true) {
            $r = array('text' => __('Você se inscreveu com sucesso.', 'wp-pro-quiz'), 'clear' => true);
        }
