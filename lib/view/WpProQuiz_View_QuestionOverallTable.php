...
class WpProQuiz_View_QuestionOverallTable extends WP_List_Table
{
    ...
    function column_name($item)
    {
        ...
        if (current_user_can('wpProQuiz_edit_quiz')) {
            $actions['wpProQuiz_edit'] = sprintf('<a href="?page=wpProQuiz&module=question&action=addEdit&quiz_id=%1$s&questionId=%2$s">' . __('Editar',
                    'wp-pro-quiz') . '</a>',
                $item['quizId'], $item['ID']);
        }

        if (current_user_can('wpProQuiz_delete_quiz')) {
            $trashUrl = wp_nonce_url(sprintf('?page=wpProQuiz&module=question&action=delete&quiz_id=%1$s&id=%2$s', $item['quizId'], $item['ID']), 'trash-wpProQuiz-question-'.$item['ID']);
