<?php

class WpProQuiz_View_QuizOverallTable extends WP_List_Table
{
    /** @var  WpProQuiz_Model_Quiz[] */
    private $quizItems;

    private $quizCount;
    private $perPage;

    /** @var  WpProQuiz_Model_Category[] */
    private $categoryItems;

    public static function getColumnDefs()
    {
        $columns = array(
            'cb' => '<input type="checkbox" />',
            'name' => __('Nome', 'wp-pro-quiz'),
            'category' => __('Categoria', 'wp-pro-quiz'),
            'shortcode' => __('Shortcode', 'wp-pro-quiz'),
            'shortcode_leaderboard' => __('Shortcode-Leaderboard', 'wp-pro-quiz')
        );

        return $columns;
    }

    function __construct($quizItems, $quizCount, $categoryItems, $perPage)
    {
        parent::__construct(array(
            'singular' => __('Questionário', 'wp-pro-quiz'),
            'plural' => __('Questionário', 'wp-pro-quiz'),
            'ajax' => false,
            'screen' => 'toplevel_page_wpproquiz'
        ));

        $this->quizItems = $quizItems;
        $this->quizCount = $quizCount;
        $this->categoryItems = $categoryItems;
        $this->perPage = $perPage;
    }

    function no_items()
    {
        _e('Não há dados disponíveis', 'wp-pro-quiz');
    }

    function column_default($item, $column_name)
    {
        return isset($item[$column_name]) ? $item[$column_name] : '';
    }

    function get_sortable_columns()
    {
        $sortable_columns = array(
            'name' => array('name', false),
            'category' => array('category', false),
        );

        return $sortable_columns;
    }

    function get_columns()
    {
        return get_column_headers(get_current_screen());
    }

    function column_name($item)
    {
        $actions = array(
            'wpProQuiz_questions' => sprintf('<a href="?page=wpProQuiz&module=question&quiz_id=%s">' . __('Questões',
                    'wp-pro-quiz') . '</a>', $item['ID']),
        );

        if (current_user_can('wpProQuiz_edit_quiz')) {
            $actions['wpProQuiz_edit'] = sprintf('<a href="?page=wpProQuiz&action=addEdit&quizId=%s">' . __('Editar',
                    'wp-pro-quiz') . '</a>', $item['ID']);
        }

        if (current_user_can('wpProQuiz_delete_quiz')) {
            $trashUrl = wp_nonce_url(sprintf('?page=wpProQuiz&action=delete&id=%s', $item['ID']), 'trash-wpProQuiz-quiz-'.$item['ID']);

            $actions['wpProQuiz_delete'] = '<a style="color: red;" href="'.$trashUrl.'">' . __('Excluir','wp-pro-quiz') . '</a>';
        }

        $actions['wpProQuiz_preview'] = sprintf('<a href="?page=wpProQuiz&module=preview&id=%s">' . __('Pré-visualização',
                'wp-pro-quiz') . '</a>', $item['ID']);

        if (current_user_can('wpProQuiz_show_statistics')) {
            $actions['wpProQuiz_statistics'] = sprintf('<a href="?page=wpProQuiz&module=statistics&id=%s">' . __('Estatísticas',
                    'wp-pro-quiz') . '</a>', $item['ID']);
        }

        if (current_user_can('wpProQuiz_toplist_edit')) {
            $actions['wpProQuiz_leaderboard'] = sprintf('<a href="?page=wpProQuiz&module=toplist&id=%s">' . __('Leaderboard',
                    'wp-pro-quiz') . '</a>', $item['ID']);
        }

        return sprintf('<a class="row-title" href="?page=wpProQuiz&module=question&quiz_id=%1$s">%2$s</a> %3$s',
            $item['ID'], $item['name'], $this->row_actions($actions));
    }

    function get_bulk_actions()
    {
        $actions = array();

        if (current_user_can('wpProQuiz_delete_quiz')) {
            $actions['delete'] = __('Excluir', 'wp-pro-quiz');
        }

        if (current_user_can('wpProQuiz_export')) {
            $actions['export'] = __('Exportar', 'wp-pro-quiz');
        }

        if (current_user_can('wpProQuiz_edit_quiz')) {
            $actions['set_category'] = __('Definir categoria', 'wp-pro-quiz');
        }

        return $actions;
    }

    function extra_tablenav($which)
    {
        if ($which != 'top') {
            return;
        }
        ?>

        <div class="alignleft actions">
            <label class="screen-reader-text" for="cat"><?php _e('Filtrar por categoria'); ?></label>
            <select name="cat" id="cat" class="postform">
                <option value="0"><?php _e('Todas as categorias'); ?> </option>
                <?php
                foreach ($this->categoryItems as $c) {
                    $isSet = isset($_GET['cat']) && $_GET['cat'] == $c->getCategoryId();

                    echo '<option class="level-0" value="' . $c->getCategoryId() . '" ' . ($isSet ? 'selected' : '') . '>' . $c->getCategoryName() . '</option>';
                }
                ?>
            </select>
            <?php submit_button(__('Filtro'), 'button', 'filter_action', false, array('id' => 'post-query-submit')); ?>
        </div>

        <?php
    }

    function column_cb($item)
    {
        return sprintf(
            '<input type="checkbox" name="quiz[]" value="%s" />', $item['ID']
        );
    }

    function prepare_items()
    {
        $this->set_pagination_args(array(
            'total_items' => $this->quizCount,
            'per_page' => $this->perPage
        ));

        $items = array();

        foreach ($this->quizItems as $q) {
            $items[] = array(
                'ID' => $q->getId(),
                'name' => $q->getName(),
                'category' => $q->getCategoryName(),
                'shortcode' => '[WpProQuiz ' . $q->getId() . ']',
                'shortcode_leaderboard' => $q->isToplistActivated() ? '[WpProQuiz_toplist ' . $q->getId() . ']' : ''
            );
        }

        $this->items = $items;
    }
}
