...
class WpProQuiz_View_FrontQuiz extends WpProQuiz_View_View
{
    ...
    private function loadButtonNames()
    {
        ...
        $names = array(
            'start_quiz' => __('Iniciar questionário', 'wp-pro-quiz'),
            'restart_quiz' => __('Reiniciar questionário', 'wp-pro-quiz'),
            'quiz_summary' => __('Resumo do questionário', 'wp-pro-quiz'),
            'finish_quiz' => __('Terminar o questionário', 'wp-pro-quiz'),
            'quiz_is_loading' => __('O questionário está carregando...', 'wp-pro-quiz'),
            'lock_box_msg' => __('Você já completou o questionário antes. Portanto, não pode iniciá-lo novamente.',
                'wp-pro-quiz'),
            'only_registered_user_msg' => __('Você deve fazer login ou se inscrever para iniciar o questionário.', 'wp-pro-quiz'),
            'prerequisite_msg' => __('Você tem que terminar o seguinte questionário para começar este questionário:', 'wp-pro-quiz')
        );
    ...
    private function showAddToplist()
    {
        ...
        <div class="wpProQuiz_addToplist" style="display: none;">
            <span style="font-weight: bold;"><?php _e('Seu resultado foi inserido na tabela de classificação',
                    'wp-pro-quiz'); ?></span>

            <div style="margin-top: 6px;">
                <div class="wpProQuiz_addToplistMessage" style="display: none;"><?php _e('Carregando',
                        'wp-pro-quiz'); ?></div>
                <div class="wpProQuiz_addBox">
                    <div>
						<span>
							<label>
                                <?php _e('Nome', 'wp-pro-quiz'); ?>: <input type="text" placeholder="<?php _e('Nome',
                    ...
                    <input class="wpProQuiz_button2" type="submit" value="<?php _e('Enviar', 'wp-pro-quiz'); ?>"
    ...
    {
        $info = '<div class="wpProQuiz_invalidate">' . __('Você deve preencher este campo.', 'wp-pro-quiz') . '</div>';

        $validateText = array(
            WpProQuiz_Model_Form::FORM_TYPE_NUMBER => __('Você deve especificar um número.', 'wp-pro-quiz'),
            WpProQuiz_Model_Form::FORM_TYPE_TEXT => __('Você deve especificar um texto.', 'wp-pro-quiz'),
            WpProQuiz_Model_Form::FORM_TYPE_EMAIL => __('Você deve especificar um endereço de e-mail.', 'wp-pro-quiz'),
            WpProQuiz_Model_Form::FORM_TYPE_DATE => __('Você deve especificar uma data.', 'wp-pro-quiz')
        );
        ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if (isset($validateText[$form->getType()])) {
                                ...
                                echo '<div class="wpProQuiz_invalidate">' . __('Você deve preencher este campo.',
                                        'wp-pro-quiz') . '</div>';
                            }
                            ?>
                        </td>
    ...
    private function showCheckPageBox($questionCount)
    {
        ...
        <div class="wpProQuiz_checkPage" style="display: none;">
            ...
            <p>
                <?php printf(__('%s de %s questões concluídas', 'wp-pro-quiz'), '<span>0</span>', $questionCount); ?>
            </p>

            <p><?php _e('Questões', 'wp-pro-quiz'); ?>:</p>
            ...
            if ($this->quiz->isFormActivated() && $this->quiz->getFormShowPosition() == WpProQuiz_Model_Quiz::QUIZ_FORM_POSITION_END
                ...
                ?>
                <h4 class="wpProQuiz_header"><?php _e('Informação', 'wp-pro-quiz'); ?></h4>
                <?php
                $this->showFormBox();
            }
    ...
    private function showInfoPageBox()
    {
        ...
        <div class="wpProQuiz_infopage" style="display: none;">
            <h4><?php _e('Informação', 'wp-pro-quiz'); ?></h4>
    ...
    private function showTimeLimitBox()
    {
        ...
        <div style="display: none;" class="wpProQuiz_time_limit">
            <div class="time"><?php _e('Limite de tempo', 'wp-pro-quiz'); ?>: <span>0</span></div>
        ...
        <div class="wpProQuiz_reviewDiv" style="display: none;">
            ...
            <div class="wpProQuiz_reviewLegend">
                <ol>
                    <li>
                        ...
                        <span class="wpProQuiz_reviewText"><?php _e('Respondida', 'wp-pro-quiz'); ?></span>
                    </li>
                    <li>
                        <span class="wpProQuiz_reviewColor" style="background-color: #FFB800;"></span>
                        <span class="wpProQuiz_reviewText"><?php _e('Análise', 'wp-pro-quiz'); ?></span>
                    </li>
                </ol>
                <div style="clear: both;"></div>
            </div>
            <div>
                <?php if ($this->quiz->getQuizModus() != WpProQuiz_Model_Quiz::QUIZ_MODUS_SINGLE) { ?>
                    <input type="button" name="review" value="<?php _e('Questão de revisão', 'wp-pro-quiz'); ?>"
    ...
    private function showResultBox($result, $questionCount)
    {
        ...
        <div style="display: none;" class="wpProQuiz_results">
            <h4 class="wpProQuiz_header"><?php _e('Resultados', 'wp-pro-quiz'); ?></h4>
            <?php if (!$this->quiz->isHideResultCorrectQuestion()) { ?>
                <p>
                    <?php printf(__('%s de %s questões respondidas corretamente', 'wp-pro-quiz'),
                        '<span class="wpProQuiz_correct_answer">0</span>', '<span>' . $questionCount . '</span>'); ?>
                </p>
            <?php }
            if (!$this->quiz->isHideResultQuizTime()) { ?>
                <p class="wpProQuiz_quiz_time">
                    <?php _e('Seu tempo: <span></span>', 'wp-pro-quiz'); ?>
                </p>
            <?php } ?>
            <p class="wpProQuiz_time_limit_expired" style="display: none;">
                <?php _e('O tempo passou', 'wp-pro-quiz'); ?>
            </p>
            <?php if (!$this->quiz->isHideResultPoints()) { ?>
                <p class="wpProQuiz_points">
                    <?php printf(__('Você atingiu %s de %s pontos, (%s)', 'wp-pro-quiz'), '<span>0</span>',
            ...
            <?php if ($this->quiz->isShowAverageResult()) { ?>
                <div class="wpProQuiz_resultTable">
                    <table>
                        <tbody>
                        <tr>
                            <td class="wpProQuiz_resultName"><?php _e('Pontuação média', 'wp-pro-quiz'); ?></td>
                        ...
                        <tr>
                            <td class="wpProQuiz_resultName"><?php _e('Sua pontuação', 'wp-pro-quiz'); ?></td>
                            <td class="wpProQuiz_resultValue">
            ...
            <div class="wpProQuiz_catOverview" <?php $this->isDisplayNone($this->quiz->isShowCategoryScore()); ?>>
                <h4><?php _e('Categorias', 'wp-pro-quiz'); ?></h4>

                <div style="margin-top: 10px;">
                    <ol>
                        <?php foreach ($this->category as $cat) {
                            if (!$cat->getCategoryId()) {
                                $cat->setCategoryName(__('Não categorizado', 'wp-pro-quiz'));
                            }
                            ?>
            ...
            <div style="margin: 10px 0px;">
                ...
                if (!$this->quiz->isBtnViewQuestionHidden()) { ?>
                    <input class="wpProQuiz_button" type="button" name="reShowQuestion"
                           value="<?php _e('Ver questões', 'wp-pro-quiz'); ?>">
                <?php } ?>
                <?php if ($this->quiz->isToplistActivated() && $this->quiz->getToplistDataShowIn() == WpProQuiz_Model_Quiz::QUIZ_TOPLIST_SHOW_IN_BUTTON) { ?>
                    <input class="wpProQuiz_button" type="button" name="showToplist"
                           value="<?php _e('Mostrar tabela de classificação', 'wp-pro-quiz'); ?>">
                <?php } ?>
            </div>
        </div>
        <?php
                ...
                foreach ($this->question as $question) {
                    ...
                    <li class="wpProQuiz_listItem" style="display: none;">
                        <div
                            ...
                            <?php printf(__('Questão %s de %s', 'wp-pro-quiz'), '<span>' . $index . '</span>',
                                '<span>' . $questionCount . '</span>'); ?>
                        </div>
                        <h5 style="<?php echo $this->quiz->isHideQuestionNumbering() ? 'display: none;' : 'display: inline-block;' ?>"
                            class="wpProQuiz_header">
                            <span><?php echo $index; ?></span>. <?php _e('Questão', 'wp-pro-quiz'); ?>
                        </h5>

                        <?php if ($this->quiz->isShowPoints()) { ?>
                            <span style="font-weight: bold; float: right;"><?php printf(__('%d pontos', 'wp-pro-quiz'),
                        ...
                        <?php if ($question->getCategoryId() && $this->quiz->isShowCategory()) { ?>
                            <div style="font-weight: bold; padding-top: 5px;">
                                <?php printf(__('Categoria: %s', 'wp-pro-quiz'),
                        ...
                        <div class="wpProQuiz_question" style="margin: 10px 0 0 0;">
                            ...
                            <?php if ($question->getAnswerType() === 'matrix_sort_answer') { ?>
                                <div class="wpProQuiz_matrixSortString">
                                    <h5 class="wpProQuiz_header"><?php _e('Classificar elementos', 'wp-pro-quiz'); ?></h5>
                            ...
                            <div class="wpProQuiz_response" style="display: none;">
                                <div style="display: none;" class="wpProQuiz_correct">
									...
									<span style="float: left;" class="wpProQuiz_respone_span">
										<?php _e('Correta', 'wp-pro-quiz'); ?>
                                            ...
                                            <span
                                                style="float: right;"><?php echo $question->getPoints() . ' / ' . $question->getPoints(); ?> <?php _e('Pontos',
                                                    'wp-pro-quiz'); ?></span>
                                ...
                                <div style="display: none;" class="wpProQuiz_incorrect">
                                    <?php if ($question->isShowPointsInBox() && $question->isAnswerPointsActivated()) { ?>
                                        <div>
									<span style="float: left;" class="wpProQuiz_respone_span">
										<?php _e('Incorreta', 'wp-pro-quiz'); ?>
									</span>
                                            <span style="float: right;"><span
                                                    class="wpProQuiz_responsePoints"></span> / <?php echo $question->getPoints(); ?> <?php _e('Pontos',
                                                    'wp-pro-quiz'); ?></span>
                        ...
                        <?php if ($question->isTipEnabled()) { ?>
                            <div class="wpProQuiz_tipp" style="display: none; position: relative;">
                                <div>
                                    <h5 style="margin: 0 0 10px;" class="wpProQuiz_header"><?php _e('Dica',
                                            'wp-pro-quiz'); ?></h5>
                        ...
                        <?php if ($this->quiz->getQuizModus() == WpProQuiz_Model_Quiz::QUIZ_MODUS_CHECK && !$this->quiz->isSkipQuestionDisabled() && $this->quiz->isShowReviewQuestion()) { ?>
                            <input type="button" name="skip" value="<?php _e('Pular questão', 'wp-pro-quiz'); ?>"
                                   class="wpProQuiz_button wpProQuiz_QuestionButton"
                                   style="float: left; margin-right: 10px !important;">
                        <?php } ?>
                        <input type="button" name="back" value="<?php _e('Voltar', 'wp-pro-quiz'); ?>"
                               class="wpProQuiz_button wpProQuiz_QuestionButton"
                               style="float: left !important; margin-right: 10px !important; display: none;">
                        <?php if ($question->isTipEnabled()) { ?>
                            <input type="button" name="tip" value="<?php _e('Dica', 'wp-pro-quiz'); ?>"
                        ...
                        <?php } ?>
                        <input type="button" name="check" value="<?php _e('Verificar', 'wp-pro-quiz'); ?>"
                               class="wpProQuiz_button wpProQuiz_QuestionButton"
                               style="float: right !important; margin-right: 10px !important; display: none;">
                        <input type="button" name="next" value="<?php _e('Próxima', 'wp-pro-quiz'); ?>"
            ...
            <?php if ($this->quiz->getQuizModus() == WpProQuiz_Model_Quiz::QUIZ_MODUS_SINGLE) { ?>
                <div>
                    <input type="button" name="wpProQuiz_pageLeft"
                           data-text="<?php echo esc_attr(__('Página %d', 'wp-pro-quiz')); ?>"
